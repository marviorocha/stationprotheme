...
if ( ! function_exists( 'odin_posted_on' ) ) {
	...
	function odin_posted_on() {
		if ( is_sticky() && is_home() && ! is_paged() ) {
			echo '<span class="featured-post">' . __( 'Sticky', 'station-pro-radio' ) . ' </span>';
		}

		// Set up and print post meta information.
		printf( '<span class="entry-date">%s <time class="entry-date" datetime="%s">%s</time></span>
		 <span class="byline">%s <span class="author vcard"><a class="url fn n" href="%s" rel="author">%s</a></span>.</span>',
			__( 'Posted in', 'station-pro-radio' ),
			esc_attr( get_the_date( 'c' ) ),
			esc_html( get_the_date() ),
			__( 'by', 'station-pro-radio' ),
...
if ( ! function_exists( 'odin_the_custom_logo' ) ) {
	...
	function odin_the_custom_logo() {
		...
		if ( ! the_custom_logo()){
			echo " <a class='navbar-brand' href=". esc_url( home_url( '/' ) ) ." title=''>". bloginfo( 'name' ) ."</a>";
		}
	}
}
