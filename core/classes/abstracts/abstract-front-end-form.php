...
abstract class Odin_Front_End_Form {
	...
	protected function process_buttons() {
		...
		if ( ! empty( $this->buttons ) ) {
			...
			$html .= '<button type="submit" class="btn btn-primary">' . __( 'Submit', 'stationprotheme' ) . '</button>';
		}
	...
	protected function display_success_message() {
		...
		if ( isset( $_GET['success'] ) && 1 == $_GET['success'] ) {
			...
			if ( ! empty( $this->success ) ) {
				...
				$html .= '<p>' . __( 'Form submitted successfully!', 'stationprotheme' ) . '</p>';
			}
			$html .= '</div>';
		}
			...
			foreach ( $this->fields as $fieldset ) {
				foreach ( $fieldset['fields'] as $field ) {
					...
					if ( $type != 'file' && $required && empty( $data[ $id ] ) ) {
						$this->set_errors( sprintf( __( '%s is required.', 'stationprotheme' ), '<strong>' . $label . '</strong>' ) );
					}

					switch ( $type ) {
						case 'email':
							if ( ! is_email( $value ) ) {
								$this->set_errors( sprintf( __( '%s must be an email address valid.', 'stationprotheme' ), '<strong>' . $label . '</strong>' ) );
							}
							break;
						case 'file':
							if ( count($files) >= 1 ) {
